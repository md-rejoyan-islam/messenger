...
export default function UserChatBar({ user, isActive, lastMessage }) {
  const shortTime = moment(lastMessage?.createdAt).startOf("").fromNow();
      ...
      <div>
        <div className="flex justify-between items-center">
          ...
          <span className="px-1 text-[14px] text-gray-500">
            {lastMessage?.createdAt ? shortTime.split("ago")[0] : ""}
          </span>
        </div>
        <div className="flex">
          <p className="text-[14px] text-gray-600 w-[200px] exsm:w-[252px] esm:w-[300px] sm:w-[390px] xsm:w-[180px] lg:w-[215px] truncate ...  ">
            {lastMessage.senderId && lastMessage.senderId !== user?._id
              ? `you: ${lastMessage?.body}`
              : lastMessage?.body}
            {!lastMessage.senderId && "start conversation"}
          </p>
        </div>
      </div>
